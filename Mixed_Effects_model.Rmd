...
df <- df %>%
  mutate(
    Green_level = as.numeric(scale(Green_level)),
    Temperature = as.numeric(scale(Temperature)),
    Relative_humidity = as.numeric(scale(Relative_humidity)),
    Blue_area_coverage = as.numeric(scale(Blue_area_coverage))
  )
...
m2_city_green <- glmmTMB(
  Mosquito_abundance ~ Green_level * Trap_type +
                       ...
                       Blue_area_coverage +
                       factor(Month) +
                      (1 + Green_level | City) +          # City random intercept + slope
...
m4_both_slopes <- glmmTMB(
  Mosquito_abundance ~ Green_level * Trap_type +
                       ...
                       Blue_area_coverage +
                       factor(Month) +
                       (1 + Green_level + Temperature | City) +
                       (1 | Location.Site:City) +
                       (1 | Mosquito_species),
  family = nbinom2(),
  data = df,
