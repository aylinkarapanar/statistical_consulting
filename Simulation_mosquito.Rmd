...
design <- design %>%
  ...
  mutate(
    ...
    # Prey abundance
    Prey_abundance = round(
      prey_base * (0.8 + 0.4 * sin(2*pi*(Month)/12)) + 
      rnorm(n(), 0, 8),
      0
    ),
    
    Host_abundance = pmax(Host_abundance, 1),
    Prey_abundance = pmax(Prey_abundance, 1)
  ) %>%
  dplyr::select(-host_base, -prey_base)
...
df <- df %>%
  mutate(
    log_mu = 2.0 +                                                        
         ...
         0.20 * log(Prey_abundance) +                     
         0.15 * log(Host_abundance) * log(Prey_abundance) +
